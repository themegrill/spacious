...
class spacious_service_widget extends WP_Widget {
	...
	function form( $instance ) {
		...
		$read_more_text = ! empty( $instance['read_more_text'] ) ? $instance['read_more_text'] : __( 'Read more', 'spacious' );
		?>
		<p>
			<label for="<?php echo $this->get_field_id( 'read_more_text' ); ?>"><?php _e( 'Read More Text:', 'spacious' ); ?></label>
			<input class="widefat" id="<?php echo $this->get_field_id( 'read_more_text' ); ?>" name="<?php echo $this->get_field_name( 'read_more_text' ); ?>" type="text" value="<?php echo esc_attr( $read_more_text ); ?>" />
		</p>
		<?php
	}
		...
		while ( $get_featured_pages->have_posts() ) :
			...
			<div class="<?php echo $service_class; ?>">
				...
				if ( has_post_thumbnail() ) {
					...
					echo '<div class="service-image">' . get_the_post_thumbnail(
						$post->ID,
						'featured',
						array(
				...
				<div class="more-link-wrap">
					<a class="more-link" title="<?php the_title_attribute(); ?>" href="<?php the_permalink(); ?>"><?php echo esc_html( $read_more_text ); ?></a>
				</div>
			</div>
			<?php ++$j; ?>
			<?php
