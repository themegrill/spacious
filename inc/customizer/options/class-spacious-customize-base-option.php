...
/**
 * Class Spacious_Customize_Base_Option
 */
class Spacious_Customize_Base_Option {
	...
	/**
	 * Spacious_Customize_Base_Option constructor.
	 */
	public function __construct() {

		// Register customizer options.
		add_action( 'customize_register', array( $this, 'spacious_customizer_options' ) );
		// Generate styles from customizer inputs.
		add_action( 'wp_enqueue_scripts', array( $this, 'spacious_enqueue_style' ) );
	...
	protected function get_menu_options() {
		...
		$menu_options         = array();
		$menu_options['none'] = esc_html__( 'None', 'spacious' );
	 ...
	 */
	public function spacious_customizer_options( $wp_customize ) {
		...
		foreach ( $this->elements as $el_key => $el_data ) :
			...
			if ( ! $is_custom_control ) {
				...
				$control_type    = 'Spacious_Customize_' . $control_type_uc . '_Control';
	 ...
	 */
	public function spacious_enqueue_style() {

		$output = $this->generate_style();

		wp_add_inline_style( 'spacious-style', $output );
	}
