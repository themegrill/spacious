<?php
/**
 * Spacious_Site_Library class to fetch the
...
class Spacious_Site_Library {
	 ...
	 *
	 * Spacious_Site_Library constructor.
	 */
	public function __construct() {
		$this->spacious_site_library_demo_packages();
	}

	/**
	 * Demo config packages
	 */
	public function spacious_site_library_demo_packages() {
		$this->spacious_site_library_get_demos();
	}
	 ...
	 */
	public static function spacious_site_library_get_demos() {
		$template = 'spacious';
		$packages = get_transient( 'spacious_site_library_theme_' . $template );
		...
		if ( false === $packages ) {
			...
			if ( ! is_wp_error( $raw_packages ) ) {
				...
				if ( $packages ) {
					set_transient( 'spacious_site_library_theme_' . $template, $packages, WEEK_IN_SECONDS );
				}
			}
		}
	 ...
	 */
	public static function spacious_site_library_page_content() {
		$template      = 'spacious';
		$output        = '';
		$demo_packages = self::spacious_site_library_get_demos();
		...
		if ( isset( $demo_packages ) ) :
			...
			$output .= '<h2 class="screen-reader-text hide-if-no-js">' . esc_html__( 'Themes list', 'spacious' ) . '</h2><!-- .screen-reader-text.hide-if-no-js -->';
			...
			foreach ( $demo_packages->demos as $demo_package_demo => $demo_package_data ) {
				...
				if ( isset( $demo_package_data->isPro ) ) {
					...
					$output .= esc_html__( 'Pro', 'spacious' );
				...
				// Import button.
				if ( ! isset( $demo_package_data->isPro ) ) {
					$output .= '<a class="btn-get-started button button-primary button-small preview install-demo-preview" href="#" data-name="themegrill-demo-importer" data-slug="themegrill-demo-importer" aria-label="' . esc_html__( 'Get started with Spacious', 'spacious' ) . '">' . esc_html__( 'Import', 'spacious' ) . '</a>';
				}
				...
				$output .= esc_html__( 'Preview', 'spacious' );
				$output .= '</a>';
				...
				if ( isset( $demo_package_data->isPro ) ) {
					...
					$output .= esc_html__( 'Buy Now', 'spacious' );
