 ...
 *
 * A unix socket wins over host/port when both are set — a case some local dev
 * environments force (MySQL listening on a port but refusing 127.0.0.1 while
 * accepting the same credentials over its socket). `WP_DB_HOST` in wp-config
 * can carry the socket in PHP's `localhost:/path/to.sock` form, which the CLI
 * does not understand — hence a separate variable rather than parsing one
 * that means different things to different clients.
 */
 ...
 * argument. A theme's serialized mods value can run tens of KB, and a
 * `FROM_BASE64('<50KB+>')` UPDATE as a single command-line argument hits
 * Windows' argv length limit (`spawn ENAMETOOLONG`) — this is the same fixture
 * mechanics ColorMag's suite hit and fixed the same way. Piping over stdin has
 * no such limit.
 */
 ...
 *
 * Deliberately NOT hardcoded as `theme_mods_spacious`: WordPress stores mods
 * per stylesheet, and Spacious Pro is documented as a **standalone** theme
 * (not a child theme) with its own `spacious-pro` stylesheet directory
 * (.themegrill-qa/knowledge.md, "Slug"/"Pro companion" section) — so a site
 * running Spacious Pro would store its mods under `theme_mods_spacious-pro`,
 * an option this suite would otherwise never touch while believing it had
 * restored the right thing. Resolved from the live `stylesheet` option
 * instead, once per process and cached; a run cannot switch themes
 * mid-flight.
 */
 ...
 * text dump does NOT reliably preserve). Call once, in global-setup, before
 * the first test runs.
 *
 ...
 * debug behind a plumbing error. A missing snapshot in particular is a
