 ...
 * (which is how ColorMag's own `playwright.config.ts` and `global-setup.ts`
 * once drifted into two different `baseURL` fallbacks that only happened to
 * agree — a mistake this suite is built to avoid from the start):
 *
 *   1. `TGQA_*`     — exported by themegrill-qa's run-suite.mjs. Wins, because
 *                     when the platform is driving, the platform decides.
 *   2. `SPACIOUS_*` — a developer's own override, from a gitignored .env.local.
 *   3. `WP_*`       — generic legacy fallback, kept for parity with the
 *                     sibling ColorMag suite this was scaffolded from; not
 *                     advertised, just honoured if present.
...
/**
 * Convenience default, so `pnpm test:e2e` needs no setup against this
 * machine's DDEV site. The DDEV project at the repo root
 * (`.ddev/config.yaml`) is named `themes`, which DDEV serves at
 * `https://themes.ddev.site` — not a `.local` domain, which nothing here
 * actually resolves.
 */
export const DEFAULT_BASE_URL = 'https://themes.ddev.site';
...
export function adminCredentials(): AdminCredentials {
  const user =
    process.env.TGQA_ADMIN_USER ?? process.env.SPACIOUS_ADMIN_USER ?? process.env.WP_ADMIN_USER;
  const password =
    process.env.TGQA_ADMIN_PASS ?? process.env.SPACIOUS_ADMIN_PASS ?? process.env.WP_ADMIN_PASSWORD;
  ...
  if (!user || !password) {
    ...
    throw new Error(
      `No admin ${missing} for ${baseUrl()}.\n` +
        ...
        '  SPACIOUS_ADMIN_USER / SPACIOUS_ADMIN_PASS (your own, in a gitignored .env.local at the theme root)\n' +
